...
> 서비스 지향 아키텍처란 비즈니스 문제를 서비스 단위로 제공하면서 플랫폼과 언어를 넘나들며 재사용하거나 복잡한 업무를 수행 할 수 있는 구조를 말한다. - [참고 링크](https://aws.amazon.com/ko/what-is/service-oriented-architecture/)
...
간단하다. `test/resources/contracts`에 계약에 맞는 폴더를 생성하고 폴더 이름에 맞는 `Base 클래스`를 생성해 빌드하면 된다.

![image](https://github.com/this-is-spear/hello-spring-contract/assets/92219795/31880d12-d6dc-4c02-ad69-2f2ef6cc4235)
...
소비자 동작은 더욱 간단하다. `@AutoConfigureStubRunner` 애너테이션을 활용해 저장소에서 계약을 가져와 테스트를 진행하면 된다.

![image](https://github.com/this-is-spear/hello-spring-contract/assets/92219795/9205cee8-88d4-4184-ae3a-c2dcbaa078f4)

소비자가 손쉽게 테스트 할 수 있도록 정말 친절하다. 자세한 코드는 `consumer` 모듈의 `FraudServiceTest` 파일을 확인하면 된다.

#### 전체 요약

- 장점
  - 공급자의 계약 검증이 간단하다. 공급자의 업데이트를 쉽게 캐치해 반영 할 수 있다.
  - 소비자의 계약 설정이 간단하다.
- 단점
  - 하지만 빌드 설정이 어렵다. 스프링 레퍼런스로는 부족하다.
  - 소비자는 공급자의 업데이트를 즉각적으로 확인 할 수 없다. 확인하기 위해서는 따로 테스트를 진행해야 한다.
